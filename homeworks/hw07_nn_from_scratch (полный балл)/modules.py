...
Original file is located at
    https://colab.research.google.com/drive/1Zs_6iNd48E_dWF1-5IsC2lOSzcDETd2P
...
class SoftMax(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        self.gradInput = grad - self.output * sum_grad
        return self.gradInput
...
class LogSoftMax(Module):
    ...
    def updateOutput(self, input):
        input_shift = input - np.max(input, axis=1, keepdims=True)
        exp = np.exp(input_shift)
        self.output = input_shift - np.log(np.sum(exp, axis=1, keepdims=True))
        return self.output

    def updateGradInput(self, input, gradOutput):
        exp = np.exp(self.output)
        sum_grad = gradOutput.sum(axis=1, keepdims=True)
        self.gradInput = gradOutput - exp * sum_grad
        return self.gradInput
...
class BatchNormalization(Module):
    ...
    def updateOutput(self, input):
        if self.training:
            ...
            if self.moving_mean is None:
                ...
                self.moving_variance = self.var.copy()
            else:
                self.moving_mean = self.alpha * self.moving_mean + (1 - self.alpha) * self.mean
                self.moving_variance = self.alpha * self.moving_variance + (1 - self.alpha) * self.var

            # Normalize the input
            self.x_normalized = (input - self.mean) / np.sqrt(self.var + self.EPS)
            self.output = self.x_normalized
        else:
            # Use moving averages for inference
            self.x_normalized = (input - self.moving_mean) / np.sqrt(self.moving_variance + self.EPS)
            self.output = self.x_normalized
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # Compute gradients
        dx = (1.0 / m) * (1.0 / np.sqrt(var_eps)) * (
            m * gradOutput
            - np.sum(gradOutput, axis=0)
            - x_norm * np.sum(gradOutput * x_norm, axis=0)
        )
...
class Dropout(Module):
    ...
    def updateGradInput(self, input, gradOutput):
        if self.training:
            ...
            # Inference: gradients pass through
...
class LeakyReLU(Module):
    ...
    def updateOutput(self, input):
        # Compute output: x if x >=0 else slope * x
        self.output = np.where(input > 0, input, input * self.slope)
        return self.output

    def updateGradInput(self, input, gradOutput):
        # Gradient is 1 for x >=0, slope otherwise
...
def adam_optimizer(variables, gradients, config, state):
    ...
    for current_layer_vars, current_layer_grads in zip(variables, gradients):
        for current_var, current_grad in zip(current_layer_vars, current_layer_grads):
            ...
            # Update first moment
            np.multiply(var_first_moment, config['beta1'], out=var_first_moment)
            np.add(var_first_moment, (1 - config['beta1']) * current_grad, out=var_first_moment)

            # Update second moment
            np.multiply(var_second_moment, config['beta2'], out=var_second_moment)
            np.add(var_second_moment, (1 - config['beta2']) * (current_grad ** 2), out=var_second_moment)

            # Compute the denominator and update
            denominator = np.sqrt(var_second_moment) + config['epsilon']
            update = lr_t * var_first_moment / denominator
            current_var -= update
...
class Conv2d(Module):
    ...
    def updateOutput(self, input):
        ...
        self.output = np.zeros((batch_size, out_channels, h, w))

        for b in range(batch_size):
            for o in range(out_channels):
                channel_sum = np.zeros((h, w))
                for i in range(in_channels):
                    corr = scipy.signal.correlate(padded_input[b, i], self.W[o, i], mode='valid')
                    channel_sum += corr
                self.output[b, o] = channel_sum + self.b[o]
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        batch_size, out_channels, h_grad, w_grad = gradOutput.shape
        ...
        for b in range(batch_size):
            for i in range(in_channels):
                channel_sum = np.zeros((h_grad, w_grad))
                for o in range(out_channels):
                    flipped_kernel = np.flip(self.W[o, i])
                    corr = scipy.signal.correlate(padded_gradOutput[b, o], flipped_kernel, mode='valid')
                    channel_sum += corr
                self.gradInput[b, i] = channel_sum
    ...
    def accGradParameters(self, input, gradOutput):
        ...
        for o in range(out_channels):
            for i in range(in_channels):
                gradW_sum = np.zeros((kh, kw))
                for b in range(batch_size):
                    corr = scipy.signal.correlate(padded_input[b, i], gradOutput[b, o], mode='valid')
                    gradW_sum += corr
                self.gradW[o, i] += gradW_sum

        np.add(self.gradb, np.sum(gradOutput, axis=(0, 2, 3)), out=self.gradb)
...
class MaxPool2d(Module):
    def __init__(self, kernel_size):
        ...
        self.kernel_size = kernel_size
        self.gradInput = None
        self.max_indices = None  # To store indices of max values
    ...
    def updateOutput(self, input):
        ...
        # Reshape and transpose to prepare for max pooling
        input_reshaped = input.reshape(n, c, h_out, k, w_out, k)
        input_transposed = input_reshaped.transpose(0, 1, 2, 4, 3, 5)
        input_view = input_transposed.reshape(n, c, h_out, w_out, k * k)

        # Compute max values and indices
        self.output = input_view.max(axis=4)
        self.max_indices = input_view.argmax(axis=4)
    ...
    def updateGradInput(self, input, gradOutput):
        ...
        # Create index grids
        n_grid, c_grid, i_grid, j_grid = np.ogrid[:n, :c, :h_out, :w_out]

        # Compute di and dj from max_indices
        di = self.max_indices // k
        dj = self.max_indices % k

        # Original positions in the input
        orig_i = i_grid * k + di
        orig_j = j_grid * k + dj
        ...
        # Assign gradients to the max positions using advanced indexing
        np.add.at(gradInput, (n_grid, c_grid, orig_i, orig_j), gradOutput)
